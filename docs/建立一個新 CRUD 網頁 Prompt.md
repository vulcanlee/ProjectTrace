...
這個方案名稱為 MyProject ，在此方案內的所有專案都會有 MyProject 的前綴名稱，例如 MyProject.Web、MyProject.Business 等等。我想要將這個 .NET Blazor 方案/專案，改成都使用 ProjectTrace 的命名規則，所有之前有用到 MyProject 的地方都改成 ProjectTrace，例如 方案名稱、各種檔案名稱、命名空間等等，當變更檔案名稱之後，原有的專案參考規則也需要保留下來，確保專案之間的引用不會出現問題。
